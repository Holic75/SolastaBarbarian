...
namespace SolastaBarbarianClass
{
    public class Main
    {
        ...
        {
            DirectoryInfo directoryInfo = new DirectoryInfo($@"{UnityModManager.modsPath}/SolastaBarbarianClass");
            FileInfo[] files = directoryInfo.GetFiles($"Translations-??.txt");

            foreach (var file in files)
            {
                var filename = $@"{UnityModManager.modsPath}/SolastaBarbarianClass/{file.Name}";
        ...
        {
            BarbarianClassBuilder.BuildAndAddClassToDB();
        }
    }
}
